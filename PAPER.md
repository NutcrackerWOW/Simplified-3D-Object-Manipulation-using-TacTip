...
*Draft manuscript targeting IEEE Robotics and Automation Letters (RA-L).*
...
How hard must a two-finger pinch squeeze to hold an object? The textbook answer —
Coulomb friction against the load, f = mg/(2μ) with a safety margin — is wrong for
soft hemispherical fingertips, and wrong in a specific, quantifiable way. In a
physics-based simulation of a two-finger hand with TacTip-style compliant fingertips
(Drake, compliant-hydroelastic contact), we measure the minimal stable squeeze force
across grasp posture, object mass, shape, friction, and fingertip stiffness, and find
the stability boundary is governed by **rolling**, not sliding: the object rotates
off the fingertip domes at an effective stability ratio μ_eff that sits 25–35 % below
the Coulomb friction of the material pair at every tested friction level. Holding
shape mass and friction fixed, geometry alone spans a 2× range of required force,
ordered exactly by the object's freedom to roll; softer fingertips — the design
direction that favors tactile sensitivity — make stability *worse* (μ_eff 0.49 → 0.68
from 25 to 100 kPa). We distill the measurements into a closed-form minimal
grip-force rule with an explicit validity envelope and failure regimes, validated on
16/16 held-out posture–mass probes, and complement it with a slip-detection study at
the real sensor's camera rate showing that threshold calibration, not detection, is
the deployable problem: the pre-slip vibration floor varies 72× across operating
conditions, and a posture-scheduled threshold recovers near-perfect detection using
only signals the controller already has. Finally, we show the measured boundary is
hold-duration-dependent — just above it the object escapes by slow rolling creep on a
5–11 s timescale, with no vibration warning — which converts the common "×1.3 safety
margin" into a measured failure mode and motivates the rule's ×2.0 margin, under
which 30/30 seeded carry runs succeed.

**Code and data:** full simulation stack, sweep datasets, and analysis scripts will
be released at [repository URL].
...
points: sensors such as the TacTip family wrap a soft hemispherical membrane around
a camera, and the same compliance that gives them their sensitivity gives the grasped
object a new way to fail — it can *roll* off the dome.
...
used" — saturates 25–35 % below the material pair's Coulomb coefficient at every
tested friction level, and a shape study at constant mass and friction spans a 2×
range of f\* ordered exactly by rolling freedom (Fig. 3). None of this is visible to
a friction-cone analysis, which predicts the same force for a sphere and a disc.

Rolling failure of soft contacts is not new *qualitatively* — the soft-finger contact
model and its limit surface bound the coupled force–torque capacity of a compliant
patch, and the grasping literature has long known that torsional load eats tangential
capacity. Our contribution is *quantitative and operational*: we measure where the
boundary actually lies for a realistic soft-fingertip geometry across posture, mass,
shape, friction, and stiffness; we identify the regimes where the friction picture
breaks entirely (a geometric support regime, a fragile seed-dependent band, a
squeeze-out ejection ceiling, and a mass ceiling where the stable window closes); and
we compress the result into a closed-form minimal-force rule with a stated envelope,
...
1. **Mechanism, quantified.** Across shape, friction, and stiffness sweeps, the
   ...
   frictions; geometry alone moves f\* by 2× at fixed friction; and softer fingertips
   lower μ_eff (0.49 → 0.68 across 25 → 100 kPa), exposing a direct
   sensitivity-versus-stability trade-off in soft tactile sensor design.
2. **A validated minimal grip-force rule.** f₁\* = (mg/2)/μ_eff(φ, θ, m) with
   fitted μ_eff, an explicit validity envelope, and automatically classified failure
   regimes. Bootstrap confidence intervals show the boundary is statistically flat in
   posture inside the envelope — the deployable core is μ_eff ≈ 0.71 − 4.2(m − 0.045)
   — and the rule scores 16/16 on held-out posture–mass probes (hold at 1.25×
   prediction, drop at 0.6×).
3. **Deployable slip-threshold calibration at the sensor's camera rate.** On a
   ...
   schedules grip force — recovers 30 TP / 1 FP / 0 FN / 29 TN at both the ideal rate
   and the 100 Hz camera rate; a model-free adaptive variant reaches a perfect score
   on this test set.
...
rolling creep, moving or motionless alike (Mann–Whitney p = 0.62), with failure times
converged in the solver time step. Rolling creep produces almost no tangential
vibration until roll-off is imminent, so no reflex can save a grasp commanded at that
margin — the ×2.0 margin in our rule is not conservatism but a measured requirement
for sustained holds.
...
formalism [Goyal et al.; Howe & Cutkosky; Kao & Cutkosky] bounds the coupled
tangential-force/torque capacity and predicts that torque demand reduces tangential
capacity. Our measurements are consistent with that picture and go beyond it in two
ways: the failing rotation here is about the *grasp axis* (the object rolls over the
dome's curvature, a geometric escape rather than in-patch spin), and we measure the
resulting boundary as a deployable scalar field μ_eff(φ, θ, m) rather than a
per-contact constitutive model.

**Grip force control and margins.** Human studies [Johansson & Westling] established
...
signals [e.g., grip-force adaptation and slip-margin estimation work]. These
approaches presume the slip limit is frictional. Our results say that for soft
domes the operative limit is rolling, sits well below the frictional one, and — near
the boundary — moves with hold duration; margins calibrated against Coulomb slip are
therefore optimistic in exactly the regime where they matter.

**Slip detection with optical tactile sensors.** Vibration- and flow-based slip
detection is well established for TacTip and GelSight-class sensors [Chorley et al.;
Ward-Cherrier et al.; James & Lepora; GelSight slip work]. Reported detectors are
typically evaluated at one or few conditions. Our expanded study reproduces the
common single-condition success and then shows the cross-condition calibration
problem (72× floor variation) that a deployment must solve; the proposed fix
(threshold scheduling over posture and load) uses no additional sensing.

**Simulation of compliant contact.** We rely on Drake's compliant-hydroelastic
contact model with the SAP discrete solver [Elandt et al.; Masterjohn et al.; Castro
et al.], which produces continuous pressure-field contact wrenches for soft
geometries and is the current standard for contact-rich manipulation simulation. We
verify our headline temporal result (creep failure time) is converged across 2×–4×
time-step refinement.

*(Reference list to be completed and verified before submission — bracketed names
are placeholders for the canonical citations.)*
...
seeds per probe, the carried-grasp failure statistics use 10 seeds per condition, and
a seed-noise audit (12 conditions × 5 independent single-seed bisections) measures
the seed-to-seed spread of f\* itself — 2–7 % of the median at 30 g (the bisection
notch) but 7–32 % at 60 g, numbers that become the weights and exclusions of §V.
...
a friction-cone analysis would infer from the measured boundary. If sliding governed,
μ_eff would equal the pair coefficient. Instead, across the friction sweep (box
object, reference posture, Table below and Fig. 3b), μ_eff/μ_pair stays at 0.66–0.77
in the mid-to-high friction range: f\* follows the Coulomb-like 1/μ scaling — friction
sets the *scale* — but the boundary binds 25–35 % early, at the roll-off.
...
**Form and fit.** With the mechanism identified, we compress the 120-condition sweep
into the physics form fixed a priori:

    f₁*(φ, θ, m) = (m·g/2) / μ_eff(φ, θ, m)
    μ_eff = 0.7069 − 0.0432·φ + 0.6019·θ + 0.0959·φ² − 1.4668·θ²
            + 0.5369·φ·θ − 4.1958·(m − 0.045)      [φ, θ in rad; m in kg]

Coefficients are fitted by weighted least squares over the **friction regime** only
(n = 60 conditions; see regimes below), with per-mass weights from the audit's
measured seed noise (σ_rel = 1.5 % at 30 g, 8.9 % at 60 g). Unweighted fit quality on
the WLS coefficients: R² = 0.60, RMSE = 0.054 over μ_eff ∈ [0.56, 0.85] (OLS reaches
R² = 0.75 but over-weights the noisier 60 g rows). The explicit mass term — μ_eff
drops ≈ 0.13 per 30 g — is essential: with the fragile rows excluded, a posture-only
fit collapses to R² ≈ 0.36, i.e. inside the envelope mass moves the boundary about
as much as posture does.

**The boundary is statistically flat in posture.** Bootstrap confidence intervals
(5,000 condition resamples) retain only the intercept (0.707, 95 % CI
[0.674, 0.739]) and the mass slope (−4.20, CI [−5.14, −3.20]); every posture
coefficient's CI straddles zero. The defensible core of the rule is therefore the
one-line

    μ_eff ≈ 0.71 − 4.2·(m − 0.045)

with the quadratic posture terms a descriptive refinement. Posture decides grasp
stability at the *regime boundaries*, not inside the safe region — the strong
PIP-flexion dependence suggested by the raw map (Fig. 4a,b) is carried by rows the
regime analysis removes.
...
- *Fragile regime* (θ = −5°, hatched ×××): the boundary's *existence* is
  ...
  force at all. These bimodal rows are excluded; they had carried the steep low-μ_eff
  end of earlier posture-only fits.
...
**Scheduled threshold (primary).** Fit log₁₀(pre-slip floor) over (φ, θ, m) on the
training episodes — the same scheduling recipe as the grip-force rule — and set the
threshold to 3× the predicted floor at the current posture and load. The floor model
is modest (R² = 0.48 ideal, 0.68 camera rate) but the ≥ 3.6× burst margin absorbs
...
trailing 2 s block-percentile with a 0.3 s guard gap (an onsetting burst cannot raise
its own baseline), floor_abs is a sensor-noise constant (3× the quietest training
floor), detections must sustain 50 ms, and the detector arms only after grasp
transients settle. With k = 100 tuned on the training split, it scores perfectly
(30/0/0/30 at both rates, latency 0.16 s). The ingredients are not optional: without
the absolute noise term it throws 19–28 false positives (ratio blow-ups on quiet
holds); without arming it fires on grip/release transients. Per-condition
calibration (28/0/2/30) remains a diagnostic upper bound but assumes training data
at every operating condition — precisely the assumption the two schemes remove.
...
(7.8 ± 1.7 s) versus 5.3–10.8 s moving (7.6 ± 1.6 s). The distributions are
statistically indistinguishable (Mann–Whitney U = 57, p = 0.62; Fig. 6a) and
insensitive to carry amplitude — the motion is not what kills the grasp. The
mechanism is the same slow rolling creep of Fig. 2: a grasp that passes any
fixed-duration stability test just above f\* still escapes on the 5–11 s scale.
...
**Design rule.** Command **f₁ = 2.0 × (mg/2)/μ_eff(φ, θ, m)** for sustained holds
and slow manipulation. At this margin, 30/30 seeded carry runs (10 seeds × three
...
Hardware validation is the natural next step. (ii) Boundary trials at 2 ms
Nyquist-limit vibration content to 250 Hz; slip trials use 1 ms. (iii) The 2 mm / 3 s
stability criterion itself defines f\*, and §VII shows the boundary moves with hold
duration; f\* values are tied to that window (the ×2.0 rule is the duration-robust
statement). (iv) Sweep boundaries use 2 seeds per probe and are seed-limited at 60 g
(spread to 32 %); the shape/material tables and carry statistics use 5 and 10 seeds.
(v) A perfect 30 + 30 slip score still only bounds each error rate below ~10 %
(rule of three); the floor model and adaptive constants are tuned to this
hand/object envelope. (vi) One grasp width (25 mm) and one base material pair;
masses validated 30–60 g; the material study gives scaling to other pairs only at
...
converts the rule into a controller that did not drop an object in 30 seeded carries.
The quantitative shape of these results — the flat-in-posture μ_eff, the support and
fragile regimes, the sensitivity–stability trade-off — is, to our knowledge, the
first deployable characterization of rolling-governed pinch stability for
TacTip-style fingertips, and every number is reproducible from the released code and
data.
...
- [ ] Verify and complete the reference list (all bracketed citations).
- [ ] Confirm Touch_Finger URDF / mesh redistribution rights for the code+data release (user homework).
- [ ] Fill repository URL in the abstract.
- [ ] Port to the RA-L LaTeX template; recheck figure sizing (3.5 in / 7.16 in columns).
